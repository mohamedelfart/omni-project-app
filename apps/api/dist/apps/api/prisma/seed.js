...
async function main() {
    ...
    const roles = await Promise.all([
        { code: client_1.RoleCode.TENANT, name: 'Tenant' },
        { code: client_1.RoleCode.LANDLORD, name: 'Landlord' },
        { code: client_1.RoleCode.ADMIN, name: 'Admin' },
        { code: client_1.RoleCode.PROVIDER, name: 'Provider' },
        { code: client_1.RoleCode.COMMAND_CENTER, name: 'Command Center' },
    ...
    await prisma.userRole.createMany({
        data: [
            { userId: landlordUser.id, roleId: getRoleId(client_1.RoleCode.LANDLORD) },
            { userId: tenantUser.id, roleId: getRoleId(client_1.RoleCode.TENANT) },
            { userId: commandCenterUser.id, roleId: getRoleId(client_1.RoleCode.ADMIN) },
            { userId: commandCenterUser.id, roleId: getRoleId(client_1.RoleCode.COMMAND_CENTER) },
            { userId: providerUser.id, roleId: getRoleId(client_1.RoleCode.PROVIDER) },
        ],
        skipDuplicates: true,
    });
    ...
    const movingProvider = await prisma.provider.upsert({
        ...
        create: {
            ...
            countryCode: 'QA',
            city: 'Doha',
            providerType: client_1.ProviderType.MOVING,
