...
/** Where Command Center suitability matched providers by city (unchanged semantics: request.city wins, else first property). */
function computeSuitabilityFilterCity(
...
/**
 * A1.7 — Single backend contract for where provider dispatch geo should anchor.
 *
 ...
 */
type UnifiedRequestExecutionSiteSource =
  | 'request-target'
  | 'property'
  | 'pickup'
  | 'dropoff'
...
function resolveUnifiedRequestExecutionSite(input: {
  city: string;
  propertyIds: string[];
